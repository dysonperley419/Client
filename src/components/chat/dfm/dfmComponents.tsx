...
export const MemberMention = ({ guild_id, user_id }: { guild_id: string | undefined, user_id: string }): JSX.Element => {
  const { getMember } = useGateway();
  const storedUsers = useUserStore((state: UserStore) => state.users);

  let name: string | undefined = getMember(guild_id, user_id)?.nick ?? undefined;
  name ??= storedUsers[user_id]?.username;
  name ??= "unknown";
  return <strong>@{name}</strong>;
};

export const RoleMention = ({ guild_id, channel_id }: { guild_id: string | undefined, channel_id: string }): JSX.Element => {
...
export const ChannelMention = ({ guild_id, channel_id }: { guild_id: string | undefined, channel_id: string }): JSX.Element => {
...
export const Emoji = ({ name, emoji_id }: { name: string, emoji_id: string }): JSX.Element => {
  ...
  if (invalid)
    return <></>; //Invalid and probably dangerous. Do not render.
